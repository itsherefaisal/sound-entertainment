    ...
    <div class="video-container w-full p-4 px-8">
        ...
        <div id="videoPlayerContainer"
            ...
            <div id="title-overlay"
                ...
                <h1 id="video-title" class="text-lg font-bold truncate"><?= $video['title']?></h1>
            </div>
            ...
            <div id="video-controls"
                ...
                <button id="custom-play-btn" class="text-white">
                    ...
                    <svg id="pause-icon" class="w-6 h-6 hidden" viewBox="0 0 24 24" fill="none" stroke="currentColor"
                        stroke-width="2" stroke-linecap="round" stroke-linejoin="round">
                ...
                <div class="volume-bar flex items-center w-[200px] gap-1">
                    ...
                    <div id="volume-control" class="relative w-full h-1 flex items-center bg-gray-700 cursor-pointer">
        ...
        <div class="mt-4 px-4 mb-8">
            <h2 id="video-details-title" class="text-2xl font-semibold"><?= $video['title']?></h2>
            <p id="video-description" class="font-medium mt-2 min-h-[40px]">
                <span class="text-gray-400 font-bold text-sm mt-2">DESCRIPTION:</span> <?= $video['description']?>
            </p>
            <p id="upload-date" class="text-gray-400 font-bold text-sm mt-2">
                RELEASE YEAR: <span class="font-medium text-gray-200"><?= $video['year']?></span>
            </p>
            <p id="upload-date" class="text-gray-400 font-bold text-sm mt-2">
                GENRE: <span class="font-medium text-gray-200"><?= $video['genre_name']?></span>
            </p>
            <p id="upload-date" class="text-gray-400 font-bold text-sm mt-2">
                LANGUAGE: <span class="font-medium text-gray-200"><?= $video['language']?></span>
            </p>
        </div>
    </div>
    <div class="more-videos px-14 w-full mt-16">
        <h3 class="text-xl font-bold mb-4">More videos.</h3>
        ...
        $related_stmt->bind_param('i',  $video_id);
        ...
        if (!empty($more_videos)):
            foreach ($more_videos as $related): ?>
            <div onclick="window.location.href = './video.php?video_id=<?= $related['video_id']?>'" class="related-card bg-gray-800 rounded-lg overflow-hidden hover:bg-gray-700 cursor-pointer transition">
                <img src="../assets/media/images/<?= htmlspecialchars($related['cover_image']) ?>"
                    class="w-full h-[200px] object-cover" alt="<?= htmlspecialchars($related['title']) ?>">
                <div class="p-3">
                    <h3 class="truncate font-semibold text-lg"><?= htmlspecialchars($related['title']) ?></h3>
                    <p class="text-gray-400 text-xs">
                        Genre: <span class="text-gray-400"><?= htmlspecialchars($related['genre_name']) ?></span>
                    </p>
                    <p class="text-gray-400 text-xs">Year: <?= htmlspecialchars($related['year']) ?></p>
                </div>
            </div>
            <?php 
            endforeach;
        else: ?>
            <p class="text-gray-400 py-24 px-16 col-span-full">No related videos found.</p>
            <?php 
        endif; ?>
        </div>
    </div>
